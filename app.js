...
/* ===== SHELLS ===== */
function sidebar(items,cur,badge){return `<aside class="w-56 border-r border-g100 flex flex-col fixed top-0 bottom-0 z-20 bg-white"><div class="px-5 py-5">${brandLogo(true,true)}${badge?`<span class="mt-2 inline-flex px-2 py-0.5 rounded-md bg-blue-tint text-blue text-[11px] font-bold">${badge}</span>`:""}</div><nav class="flex-1 px-3 space-y-1 overflow-y-auto">${items.map(it=>{const a=cur===it.id;return `<button onclick="${it.act||("go('"+it.id+"')")}" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl text-[15px] font-bold ${a?'bg-blue-tint text-blue':'text-g500 hover:text-g800 hover:bg-g100'}"><i data-lucide="${it.icon}" class="w-[16px] h-[16px]"></i><span class="flex-1 text-left">${it.label}</span>${it.dot?'<span class="inline-flex items-center justify-center text-[10px] font-bold text-white bg-red-500 rounded-full w-4 h-4">N</span>':''}</button>`;}).join("")}</nav><div class="px-3 py-3 border-t border-g100"><button onclick="logout()" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl text-[15px] font-bold text-g500 hover:text-g800 hover:bg-g100"><i data-lucide="log-out" class="w-[16px] h-[16px]"></i>로그아웃</button></div></aside>`;}
function customerShell(v){const items=[{id:"customer-dashboard",label:"캠페인 관리",icon:"layout-dashboard"},{id:"cust-channels",label:"채널 리스트",icon:"layers"},{id:"cust-apply",label:"캠페인 추가 신청",icon:"plus-circle",act:"newApply()"},{id:"workflow",label:"워크플로우",icon:"list-checks"},{id:"notes",label:"문의게시판",icon:"message-square"},{id:"calendar",label:"캠페인 일정",icon:"calendar"},{id:"settlement",label:"정산",icon:"receipt"},{id:"report",label:"리포트",icon:"bar-chart-3"},{id:"mypage",label:"마이페이지",icon:"user"}];items.forEach(function(it){if(it.id==="workflow")it.dot=wfPending(S.activeCamp);});
let b;if(v==="cust-channels")b=custChannels();else if(v==="cust-apply")b=custApply();else if(v==="workflow")b=viewWorkflow();else if(v==="notes")b=viewNotes();else if(v==="calendar")b=viewCalendar();else if(v==="settlement")b=viewSettlement();else if(v==="mypage")b=viewMyPage();else if(v==="report")b=viewReport();else b=viewCustomerDashboard();
return `<div class="flex min-h-screen bg-g50">${sidebar(items,v)}<main class="flex-1 ml-56 min-h-screen">${b}</main></div>`;}
function adminShell(v){const items=[{id:"admin-dashboard",label:"캠페인 관리",icon:"clipboard-list"},{id:"admin-workflow",label:"워크플로우 관리",icon:"list-checks"},{id:"admin-members",label:"회원DB",icon:"users"},{id:"admin-channels",label:"채널 DB",icon:"layers"}];let b=v==="admin-members"?viewAdminMembers():v==="admin-channels"?viewAdminChannels():v==="admin-workflow"?viewAdminWorkflow():viewAdminList();return `<div class="flex min-h-screen bg-g50">${sidebar(items,v,"관리자")}<main class="flex-1 ml-56 min-h-screen">${b}</main></div>`;}
