...
return `<div class="flex min-h-screen bg-g50">${sidebar(items,v,null,bell,true)}<main class="flex-1 ml-56 min-h-screen">${b}</main></div>`;}
function adminShell(v){const items=[{id:"admin-dashboard",label:"캠페인 관리",icon:"clipboard-list"},{id:"admin-todo",label:"해야 할 일",icon:"list-todo"},{id:"admin-workflow",label:"워크플로우 관리",icon:"list-checks"},{id:"admin-calendar",label:"캠페인 일정",icon:"calendar"},{id:"admin-sales",label:"매출분석",icon:"trending-up"},{id:"admin-members",label:"회원DB",icon:"users"},{id:"admin-channels",label:"채널 DB",icon:"layers"}];let b=v==="admin-members"?viewAdminMembers():v==="admin-channels"?viewAdminChannels():v==="admin-workflow"?viewAdminWorkflow():v==="admin-calendar"?viewAdminCalendar():v==="admin-sales"?viewAdminSales():v==="admin-todo"?viewTodo():viewAdminList();
const bell=buildTasks().length;
items.forEach(function(it){if(it.id==="admin-todo")it.dot=bell>0;if(it.id==="admin-workflow")it.dot=(ADM_ROWS||[]).some(function(c){return c.status==="승인 완료"&&unreadCount(c,"adm")>0;});});
return `<div class="flex min-h-screen bg-g50">${sidebar(items,v,"관리자",bell,true)}<main class="flex-1 ml-56 min-h-screen">${b}</main></div>`;}
...
if(act==="adminConti"||act==="adminVideo"){const lbl=act==="adminConti"?"콘티":"영상";return `<div ${P}><div class="text-center py-3"><div class="w-16 h-16 rounded-full bg-blue-tint grid place-items-center mx-auto mb-3"><i data-lucide="loader-circle" class="w-8 h-8 text-blue"></i></div><p class="text-[23px] font-bold text-g900 mb-1.5">크놀AD가 ${lbl}를 만들고 있어요!</p><p class="text-[16px] text-g500">완료되면 바로 알림으로 알려드릴게요. 조금만 기다려 주세요 ⏳</p>${wf.note?`<p class="text-[15px] text-orange-600 font-bold mt-3">내 수정요청: ${esc(wf.note)}</p>`:""}</div></div>`;}
if(act==="conti"||act==="video"){const isC=act==="conti";const lbl=isC?"콘티":"영상 초안";const d=asDeliver(isC?wf.conti:wf.video);if(!d)return `<div ${P}><div class="text-center py-3"><div class="w-16 h-16 rounded-full bg-blue-tint grid place-items-center mx-auto mb-3"><i data-lucide="clock" class="w-8 h-8 text-blue"></i></div><p class="text-[23px] font-bold text-g900 mb-1.5">크놀AD의 ${lbl} 전달을 기다리고 있어요</p><p class="text-[16px] text-g500">전달되면 알림으로 알려드릴게요 ⏳</p></div></div>`;return `<div ${P}><div class="text-center mb-3"><div class="w-14 h-14 rounded-full bg-emerald-50 grid place-items-center mx-auto mb-2"><i data-lucide="party-popper" class="w-7 h-7 text-emerald-500"></i></div><p class="text-[23px] font-bold text-g900">${lbl} 전달이 도착했어요!</p><p class="text-[15px] text-g500">아래 내용을 확인하고 컨펌해 주세요</p></div><div class="bg-blue-soft rounded-2xl p-5">${d.title?`<p class="text-[19px] font-bold text-g900 mb-2">${esc(d.title)}</p>`:""}${d.text?`<p class="text-[16px] text-g700 whitespace-pre-wrap leading-relaxed mb-3">${esc(d.text)}</p>`:""}${(d.fileUrl||d.link)?`<div class="flex gap-2 flex-wrap">${d.fileUrl?`<a href="${esc(d.fileUrl)}" target="_blank" rel="noreferrer" class="${BTN_GHOST} text-[13px] px-4 py-2.5"><i data-lucide="paperclip" class="w-4 h-4"></i>첨부파일 열기 ↗</a>`:""}${d.link?`<a href="${esc(d.link)}" target="_blank" rel="noreferrer" class="${BTN_GHOST} text-[13px] px-4 py-2.5"><i data-lucide="link" class="w-4 h-4"></i>링크 열기 ↗</a>`:""}</div>`:""}</div><textarea id="wfFb" rows="3" placeholder="수정 요청 사항을 입력하세요 (선택)" class="${INPUT} resize-none text-[16px]"></textarea><div class="flex gap-2"><button onclick="wfReqEdit()" class="${BTN_GHOST} flex-1">수정 요청</button><button onclick="wfConfirm()" class="${BTN} flex-1">컨펌하고 다음 →</button></div></div>`;}
return "";}
function wfStepper(step){const S7=[["문의","message-circle"],["자료","upload"],["콘티","pen-tool"],["콘티 컨펌","badge-check"],["영상","clapperboard"],["영상 컨펌","badge-check"],["완료","party-popper"]];return `<div class="flex items-center justify-between overflow-x-auto pb-1">${S7.map((st,i)=>{const idx=i+1;const done=idx<step,act=idx===step||(step>=7&&i===6);return `${i>0?`<div class="h-[3px] flex-1 min-w-[12px] mx-1 rounded-full ${idx<step||step>=7?"bg-blue":idx===step?"bg-blue/40":"bg-g200"}"></div>`:""}<div class="flex flex-col items-center gap-1.5 flex-shrink-0"><div class="w-10 h-10 rounded-full grid place-items-center flex-shrink-0 ${done||act?"text-white":"bg-g100 text-g400"}${act?" wf-glow":""}" ${done?'style="background:#10b981"':act?'style="background:#3182F6"':''}>${done?'<i data-lucide="check" class="w-5 h-5"></i>':`<i data-lucide="${st[1]}" class="w-[18px] h-[18px]"></i>`}</div><span class="text-[11px] font-bold whitespace-nowrap ${act?"text-blue":done?"text-g700":"text-g400"}">${st[0]}</span></div>`;}).join("")}</div>`;}
...
if(wf.step>=3)ev.push({ic:"upload",who:"고객",t:"소재·자료 제출",c:"blue",fileUrl:wf.dataUrl,note:wf.dataLink});
