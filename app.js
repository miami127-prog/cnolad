...
return "";}
function wfStepper(step){const S7=[["문의","message-circle"],["자료","upload"],["콘티","pen-tool"],["콘티 컨펌","badge-check"],["영상","clapperboard"],["영상 컨펌","badge-check"],["완료","party-popper"]];return `<div class="flex items-center justify-between overflow-x-auto py-5 px-2">${S7.map((st,i)=>{const idx=i+1;const done=idx<step,act=idx===step||(step>=7&&i===6);return `${i>0?`<div class="h-[3px] flex-1 min-w-[12px] mx-1 rounded-full ${idx<step||step>=7?"bg-blue":idx===step?"bg-blue/40":"bg-g200"}"></div>`:""}<div class="flex flex-col items-center gap-1.5 flex-shrink-0"><div class="w-12 h-12 rounded-full grid place-items-center flex-shrink-0 ${done||act?"text-white":"bg-white text-g400 border-2 border-g200"}${act?" wf-glow":""}" ${done?'style="background:#10b981"':act?'style="background:#3182F6"':''}>${done?'<i data-lucide="check" class="w-6 h-6"></i>':`<i data-lucide="${st[1]}" class="w-6 h-6"></i>`}</div><span class="text-[15px] font-bold whitespace-nowrap ${act?"text-blue":done?"text-g700":"text-g400"}">${st[0]}</span></div>`;}).join("")}</div>`;}
