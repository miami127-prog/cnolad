...
items.forEach(function(it){if(it.id==="admin-todo")it.dot=bell>0;if(it.id==="admin-dashboard")it.dot=(ADM_ROWS||[]).some(function(c){return c.status==="검토 대기중";});if(it.id==="admin-workflow")it.dot=(ADM_ROWS||[]).some(function(c){return c.status==="승인 완료"&&unreadCount(c,"adm")>0;});});
