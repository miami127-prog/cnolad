...
let S={view:"home",role:null,form:{},_submitted:null,_cameFrom:"apply",cust:null,wf:{step:1,edit:null}};
...
function goHome(){S.role=null;S.view="home";render();window.scrollTo(0,0);}
...
/* ===== HOME ===== */
function topbar(){return `<header class="bg-white/85 backdrop-blur-xl border-b border-g100 sticky top-0 z-40"><div class="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">${brandLogo(false,true)}<nav class="flex items-center gap-2"><button onclick="go('apply')" class="text-[15px] font-bold text-g600 hover:text-g900 px-3 py-2 rounded-2xl hover:bg-g100">캠페인 신청</button><button onclick="go('login')" class="text-[15px] font-bold px-4 py-2.5 rounded-2xl bg-g100 text-g800 hover:bg-g200">로그인</button></nav></div></header>`;}
...
/* ===== SHELLS ===== */
function sidebar(items,cur,badge){return `<aside class="w-56 border-r border-g100 flex flex-col fixed top-0 bottom-0 z-20 bg-white"><div class="px-5 py-5">${brandLogo(true,true)}${badge?`<span class="mt-2 inline-flex px-2 py-0.5 rounded-md bg-blue-tint text-blue text-[11px] font-bold">${badge}</span>`:""}</div><nav class="flex-1 px-3 space-y-1 overflow-y-auto">${items.map(it=>{const a=cur===it.id;return `<button onclick="go('${it.id}')" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl text-[15px] font-bold ${a?'bg-blue-tint text-blue':'text-g500 hover:text-g800 hover:bg-g100'}"><i data-lucide="${it.icon}" class="w-[16px] h-[16px]"></i>${it.label}</button>`;}).join("")}</nav><div class="px-3 py-3 border-t border-g100"><button onclick="logout()" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl text-[15px] font-bold text-g500 hover:text-g800 hover:bg-g100"><i data-lucide="log-out" class="w-[16px] h-[16px]"></i>로그아웃</button></div></aside>`;}
function customerShell(v){const items=[{id:"customer-dashboard",label:"내 캠페인",icon:"layout-dashboard"},{id:"cust-channels",label:"채널 리스트",icon:"layers"},{id:"cust-apply",label:"캠페인 추가 신청",icon:"plus-circle"},{id:"workflow",label:"워크플로우",icon:"git-branch"},{id:"settlement",label:"정산",icon:"receipt"},{id:"report",label:"리포트",icon:"bar-chart-3"}];
let b;if(v==="cust-channels")b=custChannels();else if(v==="cust-apply")b=custApply();else if(v==="workflow")b=viewWorkflow();else if(v==="settlement")b=viewSettlement();else if(v==="report")b=viewReport();else b=viewCustomerDashboard();
...
function viewCustomerDashboard(){const cur=S.wf.step;const bn=(S.cust&&S.cust.brand)?S.cust.brand:"브랜드";const pct=Math.round(cur/8*100);
...
return `<div class="p-8 md:p-10">${pageHeader("MY CAMPAIGN","캠페인 현황",esc(bn)+" 캠페인 진행 현황입니다.")}
<div class="grid lg:grid-cols-3 gap-4 mb-4"><div class="${CARD} lg:col-span-2 p-7"><div class="flex items-start justify-between mb-6"><div><p class="text-[14px] text-g500 mb-1">진행 캠페인</p><h2 class="text-[21px] font-bold text-g900">${esc(bn)} 캠페인</h2></div>${statusBadge(cur>=7?"리포트 완료":cur>=6?"업로드 완료":"진행중")}</div><div class="grid grid-cols-2 md:grid-cols-4 gap-4">${[["현재 단계",WF[cur]?WF[cur].t:"완료"],["담당","크놀AD 운영팀"],["광고 고지",(S.form&&S.form.ad)||"별도 협의"],["진행률",pct+"%"]].map(x=>`<div><p class="text-[14px] text-g400 mb-1">${x[0]}</p><p class="text-[16px] font-bold text-g900">${esc(x[1])}</p></div>`).join("")}</div></div>
<div class="${CARD} p-7 flex flex-col items-center justify-center text-center"><p class="text-[14px] text-g400 mb-1">진행률</p><div class="font-display text-[48px] font-bold text-g900 num leading-none">${cur}<span class="text-[24px] text-g300">/8</span></div><div class="w-full bg-g100 rounded-full h-2 mt-4 mb-3"><div class="bg-blue h-2 rounded-full transition-all" style="width:${pct}%"></div></div><button onclick="go('workflow')" class="text-[15px] text-blue font-bold">워크플로우 진행하기 →</button></div></div>
<div class="grid lg:grid-cols-2 gap-4"><div class="${CARD} p-7"><h2 class="text-[16px] font-bold text-g900 mb-4">진행 단계</h2><div class="space-y-0.5">${steps}</div></div><div class="flex flex-col gap-4"><div class="${CARD} p-7"><h2 class="text-[16px] font-bold text-g900 mb-3">현재 할 일</h2>${todo.map(it=>`<div class="flex items-center gap-3 py-1.5"><div class="w-2 h-2 rounded-full bg-blue flex-shrink-0"></div><span class="text-[16px] text-g700">${it}</span></div>`).join("")}<button onclick="go('workflow')" class="${BTN} w-full mt-4 py-3">워크플로우에서 처리하기</button></div><div class="${CARD} p-7"><h2 class="text-[16px] font-bold text-g900 mb-3">최근 업데이트</h2>${[["오늘","현재 단계: "+(WF[cur]?WF[cur].t:"완료")],["-","캠페인 승인 및 계정 발급 완료"]].map(x=>`<div class="flex items-start gap-3 py-1.5"><div class="w-1.5 h-1.5 rounded-full bg-blue mt-2 flex-shrink-0"></div><div><p class="text-[16px] font-medium text-g900">${x[1]}</p><p class="text-[13px] text-g400">${x[0]}</p></div></div>`).join("")}</div></div></div></div>`;}
function uploadZone(){return `<label class="block border-2 border-dashed border-g300 rounded-[20px] p-7 text-center cursor-pointer hover:border-blue hover:bg-blue-soft transition-all"><input type="file" multiple class="hidden" onchange="showFiles(this)"><i data-lucide="upload-cloud" class="w-9 h-9 text-g400 mx-auto mb-2"></i><p class="text-[17px] font-bold text-g700">파일을 끌어다 놓거나 클릭해서 업로드</p><p class="text-[14px] text-g400 mt-1">제품 이미지·영상·로고·가이드 등 (여러 개 가능)</p></label><div id="fileList" class="space-y-2 mt-3"></div>`;}
function showFiles(input){const el=document.getElementById("fileList");if(!el)return;const fs=[...input.files];el.innerHTML=fs.map(f=>`<div class="flex items-center gap-2 bg-g50 rounded-2xl px-3 py-2 text-[15px]"><i data-lucide="file" class="w-4 h-4 text-blue flex-shrink-0"></i><span class="flex-1 truncate text-g700">${esc(f.name)}</span><span class="text-g400 text-[12px] num">${(f.size/1024).toFixed(0)}KB</span></div>`).join("");if(window.lucide)lucide.createIcons();if(fs.length)toast(fs.length+"개 파일 선택됨");}
