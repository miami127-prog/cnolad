...
function customerShell(v){const items=[{id:"customer-dashboard",label:"캠페인 관리",icon:"layout-dashboard"},{id:"cust-channels",label:"채널 리스트",icon:"layers"},{id:"cust-apply",label:"캠페인 추가 신청",icon:"plus-circle",act:"newApply()"},{id:"workflow",label:"워크플로우",icon:"list-checks"},{id:"calendar",label:"캠페인 일정",icon:"calendar"},{id:"settlement",label:"정산",icon:"receipt"},{id:"report",label:"리포트",icon:"bar-chart-3"},{id:"mypage",label:"마이페이지",icon:"user"}];items.forEach(function(it){if(it.id==="workflow")it.dot=wfPending(S.activeCamp);});
