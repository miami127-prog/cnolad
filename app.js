...
if(act==="conti"||act==="video"){const isC=act==="conti";const lbl=isC?"콘티":"영상 초안";const d=asDeliver(isC?wf.conti:wf.video);if(!d)return `<div ${P}><div class="bg-amber-50 rounded-2xl p-4 text-[15px] text-amber-800 flex items-center gap-3"><i data-lucide="clock" class="w-5 h-5 flex-shrink-0"></i>크놀AD의 ${lbl} 전달을 기다리고 있습니다.</div></div>`;return `<div ${P}><div class="bg-blue-soft rounded-2xl p-5"><p class="text-[13px] font-bold text-blue mb-1">${lbl} 전달 도착</p>${d.title?`<p class="text-[19px] font-bold text-g900 mb-2">${esc(d.title)}</p>`:""}${d.text?`<p class="text-[16px] text-g700 whitespace-pre-wrap leading-relaxed mb-3">${esc(d.text)}</p>`:""}${(d.fileUrl||d.link)?`<div class="flex gap-2 flex-wrap">${d.fileUrl?`<a href="${esc(d.fileUrl)}" target="_blank" rel="noreferrer" class="${BTN_GHOST} text-[13px] px-4 py-2.5"><i data-lucide="paperclip" class="w-4 h-4"></i>첨부파일 열기 ↗</a>`:""}${d.link?`<a href="${esc(d.link)}" target="_blank" rel="noreferrer" class="${BTN_GHOST} text-[13px] px-4 py-2.5"><i data-lucide="link" class="w-4 h-4"></i>링크 열기 ↗</a>`:""}</div>`:""}</div><textarea id="wfFb" rows="3" placeholder="수정 요청 사항을 입력하세요 (선택)" class="${INPUT} resize-none text-[16px]"></textarea><div class="flex gap-2"><button onclick="wfReqEdit()" class="${BTN_GHOST} flex-1">수정 요청</button><button onclick="wfConfirm()" class="${BTN} flex-1">컨펌하고 다음 →</button></div></div>`;}
return "";}
